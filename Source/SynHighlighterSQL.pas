...
type
  TtkTokenKind = (tkComment, tkDatatype, tkDefaultPackage, tkException, tkFunction, tkIdentifier, tkKey, tkNull,
    tkNumber, tkSpace, tkPLSQL, tkSQLPlus, tkString, tkSymbol, tkTableName, tkFieldName, tkUnknown, tkVariable,
    tkConditionalComment, tkDelimitedIdentifier);

  TRangeState = (rsUnknown, rsComment, rsString, rsConditionalComment);

  TSQLDialect = (sqlStandard, sqlInterbase6, sqlMSSQL7, sqlMySQL, sqlOracle, sqlSybase, sqlIngres, sqlMSSQL2K,
    sqlPostgres, sqlNexus);
...
type
  ...
  private
    ...
    procedure SetFieldNames(const Value: TUnicodeStrings);
   ...
   // procedure PutFieldNamesInKeywordList;
  ...
  published
    ...
    property ConditionalCommentAttri: TSynHighlighterAttributes read fConditionalCommentAttri
      write fConditionalCommentAttri;
    property DataTypeAttri: TSynHighlighterAttributes read fDataTypeAttri write fDataTypeAttri;
    property DefaultPackageAttri: TSynHighlighterAttributes read fDefaultPackageAttri write fDefaultPackageAttri;
    property DelimitedIdentifierAttri: TSynHighlighterAttributes read fDelimitedIdentifierAttri
      write fDelimitedIdentifierAttri;
    ...
    property PLSQLAttri: TSynHighlighterAttributes read fPLSQLAttri write fPLSQLAttri;
    property SpaceAttri: TSynHighlighterAttributes read fSpaceAttri write fSpaceAttri;
    property SQLPlusAttri: TSynHighlighterAttributes read fSQLPlusAttri write fSQLPlusAttri;
    ...
    property FieldNameAttri: TSynHighlighterAttributes read fFieldNameAttri write fFieldNameAttri;
    property TableNames: TUnicodeStrings read fTableNames write SetTableNames;
    property FieldNames: TUnicodeStrings read fFieldNames write SetFieldNames;
    ...
    property SQLDialect: TSQLDialect read fDialect write SetDialect default sqlStandard;
  end;
...
const
  ...
  StandardKW: UnicodeString = 'absolute,action,active,actor,add,after,alias,all,allocate,alter,' +
    ...
    'value,varchar,variable,varying,view,virtual,visible,wait,when,where,' + 'while,with,without,work,write,year,zone';

  // ---Sybase keywords------------------------------------------------------------
  SybaseKW: UnicodeString = 'absolute,action,add,after,alias,all,allocate,alter,and,any,are,' +
  ...
  // ---Oracle---------------------------------------------------------------------
  // Oracle SQL keywords
  OracleKW: UnicodeString = 'ACCESS,ACCESSED,ACCOUNT,ACTIVATE,ACTIVE_INSTANCE_COUNT,ADD,ADMIN,ADVISE,' +
    ...
    'ORACLE_TRACE_COLLECTION_SIZE,ORACLE_TRACE_ENABLE,' + 'ORACLE_TRACE_FACILITY_NAME,ORACLE_TRACE_FACILITY_PATH,ORDER,'
    ...
    'PLSQL_NATIVE_C_COMPILER,PLSQL_NATIVE_LIBRARY_DIR,' + 'PLSQL_NATIVE_LIBRARY_SUBDIR_COUNT,PLSQL_NATIVE_LINKER,' +
    ...
    'REMOTE_DEPENDENCIES_MODE,REMOTE_LISTENER,REMOTE_LOGIN_PASSWORDFILE,' + 'REMOTE_OS_AUTHENT,REMOTE_OS_ROLES,RENAME,'
    ...
    'VARGRAPHIC,VARRAY,VIEW,WHERE,WITH,WITHOUT,WNDS,WNPS,' + 'WORKAREA_SIZE_POLICY';

  // ---Postgresql-----------------------------------------------------------------
  // Postgresql Keywords
  PostgresKW: UnicodeString = 'IF,LOOP,ABORT,ABSOLUTE,ACCESS,ACTION,ADA,ADD,ADMIN,AFTER,AGGREGATE,ALIAS' +
    ',ALLOCATE,ALTER,ANALYSE,ANALYZE,AND,ARE,AS,ASC,ASENSITIVE' +
    ',ASSERTION,ASSIGNMENT,ASYMMETRIC,AT,ATOMIC,AUTHORIZATION,BACKWARD' + ',BEFORE,BEGIN,BETWEEN' +
    ...
    ',CROSS,CUBE,CURRENT,CURRENT_PATH,CURRENT_ROLE' + ',CURSOR,CURSOR_NAME,CYCLE,DATA,DATABASE,DATETIME_INTERVAL_CODE' +
  ...
  PostgresFunctions: UnicodeString = 'abs,cbrt,ceil,ceiling,degrees,exp,floor,ln,log,mod,pi,power,radians,random,' +
    ...
    'set_byte,get_bit,set_bit,to_char,to_date,' + 'to_timestamp,to_number,age,date_part,date_trunc,extract,now,' +
    'timeofday,isfinite,area,box_intersect,center,diameter,height,isclosed,isopen,' +
    'npoints,pclose,popen,radius,width,' + 'broadcast,' +
    ...
    'version,has_table_privilege,has_database_privilege,' + 'has_function_privilege,has_language_privilege,' +
    ...
    'pg_opclass_is_visible,pg_conversion_is_visible,format_type,pg_get_viewdef,' + 'pg_get_ruledef,pg_get_indexdef,' +
  ...
  // Postgresql Types
  PostgresTypes: UnicodeString = 'smallint,integer,bigint,decimal,numeric,real,double,serial,bigserial,' +
  ...
  // PLSQL keywords
  OraclePLSQLKW: UnicodeString = 'ABORT,ACCEPT,AFTER,ARRAY,ARRAYLEN,ASSERT,ASSIGN,AT,AUTHORIZATION,' +
    ...
    'RE$RULE_HIT_LIST,RE$TABLE_ALIAS,RE$TABLE_ALIAS_LIST,' + 'RE$TABLE_VALUE,RE$TABLE_VALUE_LIST,RE$VARIABLE_TYPE,' +
    ...
    'SUBTYPE,TABAUTH,TABLES,TASK,TDO,TERMINATE,TRUE,TYPE,USE,VARYING,VIEWS,' + 'WHEN,WHILE,WORK,WRITE,XOR';

  // Oracle data types
  OracleTypes: UnicodeString = 'ANYDATA,ANYDATASET,ANYTYPE,BFILE,BINARY_INTEGER,BLOB,BOOLEAN,CHAR,CLOB,' +
    ...
    'TIMESTAMP,URIType,UROWID,VARCHAR,VARCHAR2,XDBURIType,XMLDATA,XMLType,' + 'YEAR,ZONE';

  // Oracle built in exceptions
  OracleExceptions: UnicodeString = 'ACCESS_INTO_NULL,COLLECTION_IS_NULL,CURSOR_ALREADY_OPEN,' +
  ...
  OracleFunctions: UnicodeString = 'ABS,ACOS,ADD_MONTHS,AGGREGATE,ANALYTIC,ASCII,ASCIISTR,ASIN,ATAN,ATAN2,' +
    ...
    'GET_HASH_VALUE,GET_HEADER_COUNT,GET_INDEXES,GET_INFORMATION,' + 'GET_OBJECT_NULL_VECTOR_ARG,GET_PARAMETER_VALUE,' +
    ...
    'GROUPING_ID,HEXTORAW,I_AM_A_REFRESH,INITCAP,INITIALIZE,' + 'INSTANTIATE_OFFLINE,INSTANTIATE_ONLINE,INSTR,INSTRB,' +
    ...
    'XMLAGG,XMLCOLATTVAL,XMLCONCAT,XMLELEMENT,XMLFOREST,XMLSEQUENCE,' + 'XMLTRANSFORM,XRANGE';

  OracleDefaultPackages: UnicodeString = 'DBMS_ALERT,DBMS_APPLICATION_INFO,DBMS_APPLY_ADM,DBMS_AQ,' +
    'DBMS_AQ_EXP_HISTORY_TABLES,DBMS_AQ_EXP_INDEX_TABLES,' + 'DBMS_AQ_EXP_QUEUE_TABLES,DBMS_AQ_EXP_QUEUES,' +
    'DBMS_AQ_EXP_SUBSCRIBER_TABLES,DBMS_AQ_EXP_TIMEMGR_TABLES,' +
    'DBMS_AQ_EXP_ZECURITY,DBMS_AQ_IMP_INTERNAL,DBMS_AQ_IMP_ZECURITY,' +
    'DBMS_AQ_IMPORT_INTERNAL,DBMS_AQ_SYS_EXP_ACTIONS,' + 'DBMS_AQ_SYS_EXP_INTERNAL,DBMS_AQ_SYS_IMP_INTERNAL,DBMS_AQADM,'
    ...
    'DBMS_MGWMSG,DBMS_MVIEW,DBMS_OBFUSCATION_TOOLKIT,DBMS_ODCI,' + 'DBMS_OFFLINE_OG,DBMS_OFFLINE_SNAPSHOT,DBMS_OLAP,' +
    ...
    'DBMS_STREAMS,DBMS_STREAMS_ADM,DBMS_SUMADV,DBMS_SUMMARY,' + 'DBMS_SUMREF_CHILD,DBMS_SUMREF_PARENT,DBMS_SUMREF_UTIL,'
  ...
  OracleSQLPlusCommands: UnicodeString = 'APP,APPINFO,AQ$_AGENT,AQ$_AGENT_LIST_T,AQ$_DESCRIPTOR,AQ$_POST_INFO,' +
    ...
    'UNDERLINE,UP,VAR,VARIABLE,VER,VERIFY,VERSION,VIS,VISIBLE,WHENEVER,WR,' + 'WRA,WRAP,WRAPPED';

  OracleCommentKW: UnicodeString = 'REM,REMA,REMAR,REMARK';

  // ---MS-SQL 7-------------------------------------------------------------------
  // keywords
  MSSQL7KW: UnicodeString = 'ABSOLUTE,ADD,ALL,ALTER,ANY,AS,ASC,AUTHORIZATION,AVG,BACKUP,BEGIN,' +
  ...
  // functions
  MSSQL7Functions: UnicodeString = '@@CONNECTIONS,@@CPU_BUSY,@@CURSOR_ROWS,@@DATEFIRST,@@DBTS,@@ERROR,' +
    ...
    'SUSER_SNAME,SYSTEM_USER,TAN,TEXTPTR,TEXTVALID,TYPEPROPERTY,UNICODE,' + 'UPPER,USER_ID,USER_NAME,YEAR';

  // types
  MSSQL7Types: UnicodeString = 'BINARY,BIT,CHAR,DATETIME,DECIMAL,FLOAT,IMAGE,INT,MONEY,NCHAR,NTEXT,' +
    'NUMERIC,NVARCHAR,REAL,SMALLDATETIME,SMALLINT,SMALLMONEY,SYSNAME,TEXT,' +
    'TIMESTAMP,TINYINT,UNIQUEIDENTIFIER,VARBINARY,VARCHAR';

  // ---MS-SQL2K-------------------------------------------------------------------
  // keywords
  MSSQL2000KW: UnicodeString = 'ADD,ALL,ALTER,AND,ANY,AS,ASC,AUTHORIZATION,BACKUP,' +
    'BEGIN,BETWEEN,BREAK,BROWSE,BULK,BY,CASCADE,CASE,' + 'CHECK,CHECKPOINT,CLOSE,CLUSTERED,COLLATE,' +
    'COLUMN,COMMIT,COMPUTE,CONSTRAINT,CONTAINS,CONTAINSTABLE,' + 'CONTINUE,CREATE,CROSS,CURRENT,CURSOR,DATABASE,' +
    'DBCC,DEALLOCATE,DECLARE,DEFAULT,DELETE,DENY,DESC,DISK,' + 'DISTINCT,DISTRIBUTED,DOUBLE,DROP,DUMMY,DUMP,ELSE,END,' +
    ...
    'OPTION,OR,ORDER,OUTER,OVER,PERCENT,PLAN,PRECISION,' + 'PRIMARY,PRINT,PROC,PROCEDURE,PUBLIC,RAISERROR,READ,' +
    ...
    'USE,USER,VALUES,VARYING,VIEW,WAITFOR,WEIGHT,WHEN,WHERE,WHILE,' + 'WITH,WRITETEXT';

  // functions
  MSSQL2000Functions: UnicodeString = '@@CONNECTIONS,@@CPU_BUSY,@@CURSOR_ROWS,@@DATEFIRST,@@DBTS,@@ERROR,' +
    '@@FETCH_STATUS,@@IDENTITY,@@IDLE,@@IO_BUSY,@@LANGID,@@LANGUAGE,' +
    '@@LOCK_TIMEOUT,@@MAX_CONNECTIONS,@@MAX_PRECISION,@@NESTLEVEL,@@OPTIONS,' +
    '@@PACKET_ERRORS,@@PACK_RECEIVED,@@PACK_SENT,@@PROCID,@@REMSERVER,' +
    '@@ROWCOUNT,@@SERVERNAME,@@SERVICENAME,@@SPID,@@TEXTSIZE,@@TIMETICKS,' +
    ...
    'SYSTEM_USER,TAN,TEXTPTR,TEXTVALID,TYPEPROPERTY,UNICODE,UPPER,' + 'USER_ID,USER_NAME,VAR,VARP,YEAR';

  // types
  MSSQL2000Types: UnicodeString = 'bigint,binary,bit,char,character,datetime,' + 'dec,decimal,float,image,int,' +
    'integer,money,nchar,ntext,numeric,nvarchar,real,' + 'rowversion,smalldatetime,smallint,smallmoney,' +
    'sql_variant,sysname,text,timestamp,tinyint,uniqueidentifier,' + 'varbinary,varchar';
  ...
  // types
  Interbase6Types: UnicodeString = 'BLOB,CHAR,CHARACTER,DATE,DECIMAL,DOUBLE,FLOAT,INTEGER,' +
    'NUMERIC,SMALLINT,TIME,TIMESTAMP,VARCHAR';

  // ---MySQL----------------------------------------------------------------------
  // keywords
  MySqlKW: UnicodeString = 'ACTION,AFTER,AGAINST,AGGREGATE,ALGORITHM,ALL,ALTER,ANALYZE,AND,ANY,AS,' +
  ...
  // PLSQL keywords
  MySQLPLSQLKW: UnicodeString = 'CLOSE,CONDITION,CONTINUE,CURSOR,DECLARE,DO,EXIT,FETCH,FOUND,GOTO,' +
  ...
  // Column Types
    ...
    'string,text,time,timestamp,tinyblob,tinyint,tinytext,unicode,unsigned,' + 'varbinary,varchar,year,zerofill,' +
  ...
  // Charsets
    ...
    '_latin5,_latin7,_macce,_macroman,_sjis,_swe7,_tis620,_ucs2,_ujis,_usa7,' + '_utf8,_win1250,_win1251,_win1251ukr,' +
  ...
  // functions
  MySQLFunctions: UnicodeString = 'ABS,ACOS,ADD,ADDDATE,ADDTIME,ASCII,ASIN,ATAN,ATAN2,AVG,BIN,BIT_AND,' +
  ...
  // ---Ingres---------------------------------------------------------------------
  // keywords
  IngresKW: UnicodeString = 'ABORT,ACTIVATE,ADD,ADDFORM,AFTER,AGGREGATE,ALL,ALTER,AND,APPEND,ARRAY,' +
  ...
  // types
  IngresTypes: UnicodeString = 'BYTE,C,CHAR,CHARACTER,DATE,DECIMAL,FLOAT,FLOAT4,FLOAT8,INTEGER,INTEGER1,' +
    'INTEGER2,INTEGER4,LONG,MONEY,OBJECT_KEY,SECURITY_LABEL,SHORT,SMALLINT,' + 'TABLE_KEY,TEXT,VARCHAR,VARYING';

  // functions
  IngresFunctions: UnicodeString = '_BINTIM,_CPU_MS,_DATE,_DIO_CNT,_ET_SEC,_PFAULT_CNT,_TIME,_VERSION,ABS,' +
    ...
    'TRANSACTION_STATE,TRIM,UPDATE_ROWCNT,UPDATE_SYSCAT,UPPERCASE,USERNAME,' + 'VARBYTE';

  // ---Nexus----------------------------------------------------------------------
  // keywords
  NexusKW: UnicodeString = 'ABSOLUTE,AFTER,ALTER,ANY,ASC,ASSERT,ATOMIC,' +
    'ADD,ALL,AND,AS,ASSEMBLY,AUTHORIZATION,BEFORE,' + 'BETWEEN,BINARY,BLOCK,BY,BEGIN,' + 'BLOCKSIZE,CALL,CASCADE,CAST,,'
    + 'CHARACTERS,CLR,CLOSE,CODEPAGE,COLLATION,COMMIT,CONTAINS,' + 'CROSS,CALLED,CASE,CATCH,' +
    'CHECK,COALESCE,COLLATE,COLUMN,CONSTRAINT,' + 'CREATE,CURSOR,DATA,DECLARE,' +
    'DELETE,DESC,DETERMINISTIC,DO,DROP,DAY,DEFAULT,DELETING,' +
    'DESCRIPTION,DISTINCT,EACH,ELSEIF,ENCRYPT,END,EQUIVALENT,' + 'ESCAPE,EXECUTE,EXISTS,ELSE,EMPTY,ENCRYPTION,ENGINE,' +
    ...
    'INITIALSIZE,INOUT,INSERT,INTERSECT,INTO,ITERATE,JOIN,' + 'KANA,KEY,LANGUAGE,LEAVE,LIKE,LOCALE,' +
    'LARGE,LAST,LEFT,LOCAL,MATCH,' + 'MINUTE,MODIFIES,MONTH,NAME,NATURAL,NEXT,NONSPACE,' + 'NULLIF,NATIONAL,' +
    ...
    'PRIOR,PROCEDURE,PASSWORDS,PRIMARY,REFERENCES,RELATIVE,REMOVE,RESTRICT,' + 'RETURNS,ROLLBACK,ROUTINE,READS,' +
    'REFERENCING,REPEAT,RETURN,RIGHT,ROW,SECOND,' + 'SERIALIZABLE,SET,SIMPLE,SNAPSHOT,SORT,' + 'STRING,SELECT,SIGNAL,' +
    'SOME,SQL,START,STORAGE,SYMBOLS,TABLE,' + 'TOP,TRANSACTION,TRY,THEN,' + 'TO,TRIGGER,TRUE,TYPE,UNION,UNKNOWN,UPDATE,'
    + 'UNIQUE,UNTIL,UPDATING,USE,VALUES,VARYING,' + 'VIEW,WHEN,WHILE,WITH,WORK,WHERE,WIDTH,YEAR';

  // functions
  NexusFunctions: UnicodeString = 'ABS,ATAN,ATAN2,ATN2,AVG,BOTH,BROUND,CEIL,CEILING,CHAR_LENGTH,CHARACTER_LENGTH,' +
  ...
  // types
  NexusTypes: UnicodeString = 'CHARACTER,CHAR,NULLSTRING,SHORTSTRING,SINGLECHAR,VARCHAR,' +
...
begin
  ...
  fFieldNames := TUnicodeStringList.Create;
 // TUnicodeStringList(fFieldNames).OnChange := TableNamesChanged;
 // remove this and put it to Public Function call
  ...
  fConditionalCommentAttri := TSynHighlighterAttributes.Create(SYNS_AttrConditionalComment,
    SYNS_FriendlyAttrConditionalComment);
  ...
  fDefaultPackageAttri := TSynHighlighterAttributes.Create(SYNS_AttrDefaultPackage, SYNS_FriendlyAttrDefaultPackage);
  ...
  fDelimitedIdentifierAttri := TSynHighlighterAttributes.Create(SYNS_AttrDelimitedIdentifier,
    SYNS_FriendlyAttrDelimitedIdentifier);
  ...
  fFieldNameAttri := TSynHighlighterAttributes.Create(SYNS_AttrFieldName, SYNS_FriendlyAttrFieldName);
...
begin
  ...
  begin
    ...
    begin
      if (Run > 0) or (fRange <> rsString) or ((fLine[Run] <> #39) and (fLine[Run - 1] <> '\')) then
      begin
        fRange := rsString;
        repeat
...
begin
  ...
  Result := tk in [tkDatatype, tkException, tkFunction, tkKey, tkPLSQL, tkDefaultPackage];
end;
...
function TSynSQLSyn.GetDefaultAttribute(Index: Integer): TSynHighlighterAttributes;
begin
  case Index of
    SYN_ATTR_COMMENT:
...
procedure TSynSQLSyn.PutFieldNamesInKeywordList;
var
  i: Integer;
  Entry: TSynHashEntry;
begin
  ...
  begin
    Entry := fKeywords[HashKey(PWideChar(fFieldNames[i]))];
    while Assigned(Entry) do
    begin
      if SynWideLowerCase(Entry.Keyword) = SynWideLowerCase(fFieldNames[i]) then
        Break;
      Entry := Entry.Next;
    end;
    if not Assigned(Entry) then
...
procedure TSynSQLSyn.SetFieldNames(const Value: TUnicodeStrings);
begin
...
begin
  ...
  case fDialect of
    sqlPostgres:
      Result := '-- ANSI SQL sample source'#13#10 + 'SELECT *'#13#10 + 'FROM planets'#13#10 +
        'WHERE diameter < 13000'#13#10 + '  AND name <> ''Earth''';
    sqlStandard:
      Result := '-- ANSI SQL sample source'#13#10 + 'SELECT *'#13#10 + 'FROM planets'#13#10 +
        'WHERE diameter < 13000'#13#10 + '  AND name <> ''Earth''';
    sqlInterbase6:
      Result := '/* Interbase sample source */'#13#10 + 'SET TERM !! ;'#13#10 + #13#10 +
        'CREATE PROCEDURE HelloWorld(P_MSG VARCHAR(80)) AS'#13#10 + 'BEGIN'#13#10 +
        '  EXECUTE PROCEDURE WRITELN(:P_MSG);'#13#10 + 'END !!'#13#10 + #13#10 + 'SET TERM ; !!';
    sqlMySQL:
      Result := '/* MySQL sample source*/'#13#10 + 'SET @variable = 1;'#13#10 + #13#10 +
        'CREATE /*!32302 TEMPORARY */ TABLE t (a INT);'#13#10 + #13#10 + 'CREATE TABLE sample ('#13#10 +
        '        id INT NOT NULL,'#13#10 + '        first_name CHAR(30) NOT NULL,'#13#10 +
        '        PRIMARY KEY (id),'#13#10 + '        INDEX name (first_name));'#13#10 + #13#10 +
        'SELECT DATE_ADD(''1997-12-31 23:59:59'','#13#10 + '        INTERVAL 1 SECOND);'#13#10 + #13#10 +
        '# End of sample';
    sqlOracle:
      Result := 'PROMPT Oracle sample source'#13#10 + 'declare'#13#10 + '  x varchar2(2000);'#13#10 +
        'begin   -- Show some text here'#13#10 + '  select to_char(count(*)) into x'#13#10 + '  from tab;'#13#10 +
        #13#10 + '  dbms_output.put_line(''Hello World: '' || x);'#13#10 + 'exception'#13#10 +
        '  when others then'#13#10 + '    null;'#13#10 + 'end;';
    sqlSybase:
      Result := '/* SyBase example source */'#13#10 + 'declare @Integer        int'#13#10 + #13#10 +
        '/* Good for positive numbers only. */'#13#10 + 'select @Integer = 1000'#13#10 + #13#10 +
        'select "Positives Only" ='#13#10 + '  right(replicate("0",12) + '#13#10 +
        '    convert(varchar, @Integer),12)'#13#10 + #13#10 + '/* Good for positive and negative numbers. */'#13#10 +
        'select @Integer = -1000'#13#10 + #13#10 + 'select "Both Signs" ='#13#10 +
        '  substring( "- +", (sign(@Integer) + 2), 1) +'#13#10 + '  right(replicate("0",12) + '#13#10 +
        '    convert(varchar, abs(@Integer)),12)'#13#10 + #13#10 + 'select @Integer = 1000'#13#10 + #13#10 +
        'select "Both Signs" ='#13#10 + '  substring( "- +", (sign(@Integer) + 2), 1) +'#13#10 +
        '  right(replicate("0",12) + '#13#10 + '    convert(varchar, abs(@Integer)),12)'#13#10 + #13#10 + 'go';
    sqlIngres:
      Result := '/* Ingres example source */'#13#10 + 'DELETE'#13#10 + 'FROM t1'#13#10 + 'WHERE EXISTS'#13#10 +
        '(SELECT t2.column1, t2.column2'#13#10 + 'FROM t2'#13#10 + 'WHERE t1.column1 = t2.column1 and'#13#10 +
        't1.column2 = t2.column2)';
    sqlMSSQL7:
      Result := '/* SQL Server 7 example source */'#13#10 + 'SET QUOTED_IDENTIFIER ON'#13#10 + 'GO'#13#10 +
        'SET ANSI_NULLS OFF'#13#10 + 'GO'#13#10 + #13#10 + '/* Object:  Stored Procedure dbo.sp_PPQInsertOrder */'#13#10
        + 'CREATE PROCEDURE sp_PPQInsertOrder'#13#10 + '  @Name    varchar(25),'#13#10 +
        '  @Address varchar(255),'#13#10 + '  @ZipCode varchar(15)'#13#10 + 'AS'#13#10 +
        '  INSERT INTO PPQOrders(Name, Address, ZipCode, OrderDate)'#13#10 +
        '  VALUES (@Name, @Address, @ZipCode, GetDate())'#13#10 + #13#10 + '  SELECT SCOPE_IDENTITY()'#13#10 + 'GO';
    sqlMSSQL2K:
      Result := '/* SQL Server2000 example source */'#13#10 + 'SET QUOTED_IDENTIFIER ON'#13#10 + 'GO'#13#10 +
        'SET ANSI_NULLS OFF'#13#10 + 'GO'#13#10 + #13#10 + '/* Object:  Stored Procedure dbo.sp_PPQInsertOrder */'#13#10
        + 'CREATE PROCEDURE sp_PPQInsertOrder'#13#10 + '  @Name    varchar(25),'#13#10 +
        '  @Address varchar(255),'#13#10 + '  @ZipCode varchar(15)'#13#10 + 'AS'#13#10 +
        '  INSERT INTO PPQOrders(Name, Address, ZipCode, OrderDate)'#13#10 +
